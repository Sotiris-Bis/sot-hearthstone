  ...
  <div class="container">
    <b-container fluid>
      <b-row>
        ...
        <b-col lg="6" class="my-1">
          <p>Select Mana cost</p>
          <b-form-select v-model="selectedManaCost" :options="manaCostOptions" class="select transparent"> </b-form-select>
        </b-col>
      </b-row>
    </b-container>
    <div class="container">
...
export default {
  ...
  computed: {
    ...
    filteredCards() {
      if (this.selected == null && this.selectedManaCost == null) {
        return this.arenaCards;
      } else if (this.selected !== null && this.selectedManaCost == null) {
        return this.arenaCards.filter(card => card.classId == this.selected || card.multiClassIds.includes(this.selected));
      } else if (this.selected == null && this.selectedManaCost !== null) {
        return this.arenaCards.filter(card => card.manaCost == this.selectedManaCost);
      } else
        return this.arenaCards
          .filter(card => card.classId == this.selected || card.multiClassIds.includes(this.selected))
          .filter(card => card.manaCost == this.selectedManaCost);
    },
    ...mapState({
    ...
    totalQuilboars() {
      return this.filteredCards.filter(card => card.minionTypeId == 43);
    }
  }
};
</script>
